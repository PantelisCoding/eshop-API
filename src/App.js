    ...
    <>
      ...
      {showSignOut && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: '#FFFFFF', borderRadius: '0', padding: '32px 36px', textAlign: 'center', maxWidth: '340px', width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
              <button onClick={() => setShowSignOut(false)} style={{ padding: '9px 22px', borderRadius: '0', border: '1.5px solid #94A3B8', background: '#FFFFFF', color: '#374151', fontSize: '14px', fontWeight: '500', cursor: 'pointer' }}>Cancel</button>
              <button onClick={() => { setLoggedInUser(null); setShowSignOut(false); window.location.href = '/signin'; }} style={{ padding: '9px 22px', borderRadius: '0', border: 'none', background: '#DC2626', color: '#FFFFFF', fontSize: '14px', fontWeight: '500', cursor: 'pointer' }}>Sign out</button>
            </div>
          </div>
        </div>
      )}
      ...
      <nav style={{
        ...
        {/* Logo */}
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '9px', textDecoration: 'none', marginRight: '36px' }}>
          <div style={{ width: '38px', height: '38px', background: '#DC2626', borderRadius: '0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        {/* Nav links */}
        <div style={{ display: 'flex', gap: '2px', marginRight: '24px' }}>
          {navLinks.map(link => {
            ...
            return (
              <Link key={link.path} to={link.path} className="nav-link" style={{
                ...
                background: active ? '#FEF2F2' : 'transparent', transition: 'all 0.15s ease'
              }}>{link.label}</Link>
            );
          })}
        </div>
        ...
        <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', width: '560px' }}>
          <input
          ...
          {searchQuery ? (
            ...
            <svg style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} width="18" height="18" viewBox="0 0 16 16" fill="none">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginLeft: 'auto' }}>
          ...
          <button onClick={onFavoritesClick} className="fav-nav-btn" style={{
            ...
            color: '#374151', cursor: 'pointer', fontSize: '14px', fontWeight: '500', borderRadius: '0',
          }}>
            ...
            Favorites
            {favoritesCount > 0 && (
              <span style={{ background: '#DC2626', color: '#FFFFFF', fontSize: '11px', fontWeight: '700', padding: '1px 7px', borderRadius: '2px', minWidth: '20px', textAlign: 'center' }}>{favoritesCount}</span>
            )}
          </button>
          ...
          <button onClick={onCartClick} className="cart-btn" style={{
            ...
            Cart
            {cartCount > 0 && (
              <span style={{ background: '#DC2626', color: '#FFFFFF', fontSize: '11px', fontWeight: '700', padding: '1px 7px', borderRadius: '2px', minWidth: '20px', textAlign: 'center' }}>{cartCount}</span>
            )}
          </button>
          ...
          {loggedInUser ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '6px 12px', borderRadius: '0', background: '#F8FAFC' }}>
              ...
              <button onClick={() => setShowSignOut(true)} className="signout-btn" style={{ fontSize: '13px', padding: '7px 14px', borderRadius: '0', border: '1px solid #94A3B8', background: 'transparent', color: '#DC2626', cursor: 'pointer', fontWeight: '500' }}>Sign out</button>
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 14px', border: '1.5px solid #94A3B8', borderRadius: '0', background: '#FFFFFF' }}>
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 88px)', background: '#F8FAFC', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', padding: '40px' }}>
        ...
        <button onClick={() => navigate('/')} style={{ padding: '11px 28px', border: 'none', background: '#DC2626', color: '#FFFFFF', fontSize: '14px', fontWeight: '600', cursor: 'pointer', borderRadius: '0' }}>Go home</button>
      </div>
    </div>
  );
}
